CREATE TABLE [edfi].[ParentAddress] (
    ...
    [BeginDate]                DATE           NULL,
    [EndDate]                  DATE           NULL,
    CONSTRAINT [PK_ParentAddress] PRIMARY KEY CLUSTERED ([ParentUSI] ASC, [AddressTypeId] ASC) WITH (DATA_COMPRESSION = PAGE),
    CONSTRAINT [FK_ParentAddress_AddressType] FOREIGN KEY ([AddressTypeId]) REFERENCES [edfi].[AddressType] ([AddressTypeId]),
    CONSTRAINT [FK_ParentAddress_CountryCodeType] FOREIGN KEY ([CountryCodeTypeId]) REFERENCES [edfi].[CountryCodeType] ([CountryCodeTypeId]),
    CONSTRAINT [FK_ParentAddress_Parent] FOREIGN KEY ([ParentUSI]) REFERENCES [edfi].[Parent] ([ParentUSI]),
    CONSTRAINT [FK_ParentAddress_StateAbbreviationType] FOREIGN KEY ([StateAbbreviationTypeId]) REFERENCES [edfi].[StateAbbreviationType] ([StateAbbreviationTypeId])
);


GO
EXECUTE sp_addextendedproperty @name = N'MS_Description', @value = N'Parent Identity Column', @level0type = N'SCHEMA', @level0name = N'edfi', @level1type = N'TABLE', @level1name = N'ParentAddress', @level2type = N'COLUMN', @level2name = N'ParentUSI';
