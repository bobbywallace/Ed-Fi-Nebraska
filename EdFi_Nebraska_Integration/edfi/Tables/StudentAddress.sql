CREATE TABLE [edfi].[StudentAddress] (
    ...
    [BeginDate]                DATE           NULL,
    [EndDate]                  DATE           NULL,
    CONSTRAINT [PK_StudentAddress] PRIMARY KEY CLUSTERED ([StudentUSI] ASC, [AddressTypeId] ASC) WITH (DATA_COMPRESSION = PAGE),
    CONSTRAINT [FK_StudentAddress_AddressType] FOREIGN KEY ([AddressTypeId]) REFERENCES [edfi].[AddressType] ([AddressTypeId]),
    CONSTRAINT [FK_StudentAddress_CountryCode] FOREIGN KEY ([CountryCodeTypeId]) REFERENCES [edfi].[CountryCodeType] ([CountryCodeTypeId]),
    CONSTRAINT [FK_StudentAddress_StateAbbreviationType] FOREIGN KEY ([StateAbbreviationTypeId]) REFERENCES [edfi].[StateAbbreviationType] ([StateAbbreviationTypeId]),
    CONSTRAINT [FK_StudentAddress_Student] FOREIGN KEY ([StudentUSI]) REFERENCES [edfi].[Student] ([StudentUSI])
);


GO
EXECUTE sp_addextendedproperty @name = N'MS_Description', @value = N'Address Identity Column', @level0type = N'SCHEMA', @level0name = N'edfi', @level1type = N'TABLE', @level1name = N'StudentAddress', @level2type = N'COLUMN', @level2name = N'StudentUSI';
