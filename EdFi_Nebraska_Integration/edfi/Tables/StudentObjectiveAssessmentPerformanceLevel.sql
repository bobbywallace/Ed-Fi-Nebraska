CREATE TABLE [edfi].[StudentObjectiveAssessmentPerformanceLevel] (
    ...
    CONSTRAINT [PK_StudentObjectiveAssessmentPerformanceLevel] PRIMARY KEY CLUSTERED ([StudentUSI] ASC, [ObjectiveItem] ASC, [AssessmentTitle] ASC, [AcademicSubjectTypeId] ASC, [AssessedGradeLevelTypeId] ASC, [Version] ASC, [AdministrationDate] ASC, [PerformanceLevelDescriptorId] ASC) WITH (DATA_COMPRESSION = PAGE),
    CONSTRAINT [FK_StudentObjectiveAssessmentPerformanceLevel_PerformanceLevelDescriptor] FOREIGN KEY ([PerformanceLevelDescriptorId]) REFERENCES [edfi].[PerformanceLevelDescriptor] ([PerformanceLevelDescriptorId]),
    CONSTRAINT [FK_StudentObjectiveAssessmentPerformanceLevel_StudentObjectiveAssessment] FOREIGN KEY ([StudentUSI], [ObjectiveItem], [AssessmentTitle], [AcademicSubjectTypeId], [AssessedGradeLevelTypeId], [Version], [AdministrationDate]) REFERENCES [edfi].[StudentObjectiveAssessment] ([StudentUSI], [ObjectiveItem], [AssessmentTitle], [AcademicSubjectTypeId], [AssessedGradeLevelTypeId], [Version], [AdministrationDate])
);


GO
EXECUTE sp_addextendedproperty @name = N'MS_Description', @value = N'Student Identity Column', @level0type = N'SCHEMA', @level0name = N'edfi', @level1type = N'TABLE', @level1name = N'StudentObjectiveAssessmentPerformanceLevel', @level2type = N'COLUMN', @level2name = N'StudentUSI';
