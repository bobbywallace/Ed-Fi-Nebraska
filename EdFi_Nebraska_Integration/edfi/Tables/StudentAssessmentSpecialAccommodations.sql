CREATE TABLE [edfi].[StudentAssessmentSpecialAccommodations] (
    ...
    CONSTRAINT [PK_StudentAssessmentSpecialAccommodations] PRIMARY KEY CLUSTERED ([StudentUSI] ASC, [AssessmentTitle] ASC, [AcademicSubjectTypeId] ASC, [AssessedGradeLevelTypeId] ASC, [Version] ASC, [AdministrationDate] ASC, [SpecialAccommodationsTypeId] ASC) WITH (DATA_COMPRESSION = PAGE),
    CONSTRAINT [FK_SpecialAccommodationsType_StudentAssessmentSpecialAccommodations] FOREIGN KEY ([SpecialAccommodationsTypeId]) REFERENCES [edfi].[SpecialAccommodationsType] ([SpecialAccommodationsTypeId])
);


GO
EXECUTE sp_addextendedproperty @name = N'MS_Description', @value = N'Student Identity Column', @level0type = N'SCHEMA', @level0name = N'edfi', @level1type = N'TABLE', @level1name = N'StudentAssessmentSpecialAccommodations', @level2type = N'COLUMN', @level2name = N'StudentUSI';
